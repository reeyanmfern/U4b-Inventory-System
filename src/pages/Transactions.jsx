    ...
    <div className="flex flex-col h-full">
      ...
      <div className="page-header px-6 py-6 flex-shrink-0">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[#5A96BE] text-xs font-semibold uppercase tracking-widest mb-1">Sales</p>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={openSaleModal} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#3C78A0] hover:bg-[#5A96BE] text-white text-sm font-semibold transition-colors shadow-lg shadow-[#3C78A0]/30">
      ...
      <div className="flex-1 p-6 overflow-y-auto scrollbar-thin space-y-5">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: 'Total Sales', value: totalSales, color: 'text-[#2C5F80]', bar: 'bg-[#3C78A0]' },
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="relative">
              ...
              <input type="text" placeholder="Search person, product, event…" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-9 pr-4 py-2 border border-slate-200 rounded-lg text-sm bg-slate-50 focus:bg-white focus:ring-2 focus:ring-[#3C78A0] outline-none" />
            </div>
            <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className="px-3 py-2 border border-slate-200 rounded-lg text-sm bg-slate-50 focus:ring-2 focus:ring-[#3C78A0] outline-none" />
            <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} className="px-3 py-2 border border-slate-200 rounded-lg text-sm bg-slate-50 focus:ring-2 focus:ring-[#3C78A0] outline-none" />
          </div>
          <div className="flex justify-between items-center mt-3">
            <p className="text-xs text-slate-400">{filteredTransactions.length} of {transactions.length} records</p>
            {(searchTerm || dateFrom || dateTo) && <button onClick={() => { setSearchTerm(''); setDateFrom(''); setDateTo('') }} className="text-xs text-[#3C78A0] hover:text-[#2C5F80] font-semibold">Clear filters</button>}
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filteredTransactions.length === 0 ? (
                  ...
                  <tr key={tx.id} className={`${i % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'} hover:bg-[#EBF4FA]/30 transition-colors`}>
                    ...
                    <td className="py-3 px-4">
                      <p className="font-mono font-bold text-[#3C78A0] text-xs">{tx.products?.code || tx.products?.name || '—'}</p>
                    ...
                    <td className="py-3 px-4">
                      {tx.receipt_url ? (
                        <a href={tx.receipt_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-[#3C78A0] hover:text-[#2C5F80] text-xs font-semibold">
                    ...
                    <td className="py-3 px-4">
                      <div className="flex gap-1">
                        <button onClick={() => openEditModal(tx)} className="p-1.5 text-slate-400 hover:text-[#3C78A0] hover:bg-[#EBF4FA] rounded-lg transition-colors">
            ...
            <form onSubmit={handleRecordSale} className="p-6 space-y-4">
              ...
              <div>
                ...
                {receiptPreview ? (
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button type="button" onClick={() => cameraInputRef.current?.click()} className="flex flex-col items-center gap-2 py-5 border-2 border-dashed border-slate-200 rounded-xl hover:border-[#A0C8DC] hover:bg-[#EBF4FA]/50 text-slate-500 hover:text-[#3C78A0] transition-colors">
                    ...
                    <button type="button" onClick={() => galleryInputRef.current?.click()} className="flex flex-col items-center gap-2 py-5 border-2 border-dashed border-slate-200 rounded-xl hover:border-[#A0C8DC] hover:bg-[#EBF4FA]/50 text-slate-500 hover:text-[#3C78A0] transition-colors">
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button type="submit" disabled={submitting || uploading} className="flex-1 py-2.5 rounded-xl bg-[#3C78A0] hover:bg-[#2C5F80] text-white font-semibold text-sm transition-colors disabled:opacity-60 shadow-lg shadow-[#3C78A0]/20">
          ...
          <div className="bg-white rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto shadow-2xl scrollbar-thin">
            ...
            <form onSubmit={handleEditSubmit} className="p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button type="submit" disabled={editSubmitting} className="flex-1 py-2.5 rounded-xl bg-[#3C78A0] hover:bg-[#2C5F80] text-white font-semibold text-sm transition-colors disabled:opacity-60">
